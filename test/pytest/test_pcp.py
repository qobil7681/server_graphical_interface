...
async def test_pcp_open_error(transport, archive):
    ...
    await transport.check_open('metrics1', source=str(archive), limit=-10, problem='protocol-error',
                               reply_keys={'message': 'invalid "limit" value: -10'})
    invalid_timestamp_size = sys.maxsize * 1000 + 3
    await transport.check_open('metrics1', source=str(archive), timestamp=invalid_timestamp_size,
                               problem='protocol-error',
...
async def test_pcp_discrete_metric(transport, discrete_metric_archive):
    _ = await transport.check_open('metrics1', source=str(discrete_metric_archive),
                                    metrics=[{"name": "mock.value"}])
    _, data = await transport.next_frame()
...
async def test_pcp_unsupported_metric(transport, unsupported_metric_archive):
    ...
    _ = await transport.check_open('metrics1', source=str(unsupported_metric_archive),
                                    metrics=[{"name": "mock.value"}])
    _, data = await transport.next_frame()
